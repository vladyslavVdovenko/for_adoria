...
class GMM_multidim(MyDataset):
    def create_data(self, centers, num=100, std=0.7):
        '''
        Создать набор данных для кластеризации

             : param центров: массив центральных точек кластера. Если центральная точка является двумерной, каждый сгенерированный образец является двумерным.
             : param num: количество образцов
             : param std: стандартное отклонение выборок в каждом кластере
             : return: Набор данных, используемый для кластеризации. Является кортежем, первый элемент является выборочным набором, а второй элемент является истинной меткой классификации кластера выборочного набора
        '''
...
class tableDataset(MyDataset):
    def __init__(self, args):
        ...
        self._input_dim = 18  # Change dimension here
        self.filename = 'synt'
        self.data_dir = """C:/Users/vladi/Desktop/PRIMEX/DeepDPM-main/custom_data/SYNT"""
    ...
    def get_data(self):
        ...
        return train_data, self.get_labels

    def get_labels(self):
        return None

    def get_train_data(self):
        df = pd.read_csv(self.data_dir + '/synt_0.csv')
    ...
    def get_test_data(self):
        return pd.read_csv(self.data_dir + '/synt_0.csv')
...
class GMM_dataset(MyDataset):
    ...
    def __init__(self, args, samples=None, labels=None):
        ...
        if samples:
            ...
            self.mus = [  # possible number of clusters
                    # torch.tensor([-5., -5., -1]),
                    # torch.tensor([-5., -3., 0]),
                    torch.tensor([-5., 0.]),
                    torch.tensor([-5., 3.]),
            ...
            self.samples, self.labels = self._sample(1000)
            # self.samples, self.labels = self._sample(10000)
            h = 1

    def create_data(self, num=1000, std=0.7):
        '''
        Создать набор данных для кластеризации
        ...
        tensor_samples = torch.cat(
            new_X, out=torch.Tensor(num, self.input_dim)
        ).reshape(num, -1)

        return tensor_samples, torch.tensor(labels_true)
